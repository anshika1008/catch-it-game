...
void instructions()
{
  ...
  outtextxy(7, 160, "2. Catch as many boll as you can.");

  setcolor(4);

  outtextxy(7, 200, "3. Avoid black colored boll .");
...
void game_play()
{
  int i, speed = 5, k = 5, x, crashed = 0, life = 3, score = 0, random_color = 1, random_style = 1, basket_x = getmaxx()/2-80, basket_y = getmaxy()/2+80, boll_x = 50, boll_y = 45, pos = 0;
...
/*line(0, 30, getmaxx(), 30);
  ...
  // Random fill styles and colors for bolls.

  setfillstyle(random_style, random_color);

  // Ccnstructs boll and fills it with the fill pattern provided.

  fillellipse(boll_x, boll_y, 20,20);
  ...
  outtextxy(20, 0, life_display);
	...
	switch(getch())
	{
     ...
     // Checking whether boll falls within the basket.

     if(boll_x + 80 >= x && boll_x - 100 <= x && boll_y + 45 >= basket_y && boll_y - 23 <= basket_y + 45)
     {
	// Game play ends only when a black boll falls on the basket.
	...
	else
	{
     ...
     if(boll_y >= getmaxy() - 90)
     {
	boll_x = random(getmaxx() - 30);
...
void intro()
{
  ...
  for(i=-180,j=getmaxx()+30;i<=getmaxx()+40;i++,j--)
  {
   ...
   {
    setfillstyle(1, 7);
    fillellipse(getmaxx()/2+60, i, 20, 20);
    setfillstyle(1, 5);
